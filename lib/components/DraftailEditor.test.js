...
describe('DraftailEditor', () => {
    ...
    it('#maxListNesting', () => {
        behavior.generateListNestingStyles = jest.fn(() => '.t { fill: red; }');
        expect(
            shallow(<DraftailEditor maxListNesting={6} />),
        ).toMatchSnapshot();
        expect(behavior.generateListNestingStyles).toHaveBeenCalledWith(6);
        behavior.generateListNestingStyles.mockRestore();
    });
