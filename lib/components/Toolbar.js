...
const getButtonLabel = (type, label = LABELS[type]) => label;

const getButtonIcon = (type, icon = ICONS[type]) => icon;
        ...
        <ToolbarGroup>
            {inlineStyles.map(style => (
                <ToolbarButton
                    ...
                    label={getButtonLabel(style.type, style.label)}
                    title={getButtonTitle(style.type, style.description)}
                    icon={getButtonIcon(style.type, style.icon)}
        ...
        <ToolbarGroup>
            {blockTypes.map(block => (
                <ToolbarButton
                    ...
                    label={getButtonLabel(block.type, block.label)}
                    title={getButtonTitle(block.type, block.description)}
                    icon={getButtonIcon(block.type, block.icon)}
        ...
        <ToolbarGroup>
            {entityTypes.map(entity => (
                <ToolbarButton
                    ...
                    label={getButtonLabel(entity.type, entity.label)}
                    title={getButtonTitle(entity.type, entity.description)}
                    icon={getButtonIcon(entity.type, entity.icon)}
                />
            ))}
        </ToolbarGroup>
