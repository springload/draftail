...
class DraftailEditor extends Component {
    ...
    handleKeyCommand(command) {
        const { editorState } = this.state;
        const isBlockCommand = Object.keys(BLOCK_TYPE).find(key => BLOCK_TYPE[key] === command);
        const isInlineCommand = Object.keys(INLINE_STYLE).find(key => INLINE_STYLE[key] === command);
        let newState;
        let ret = false;
