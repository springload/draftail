...
const defaultProps = {
  ...
  onSave: null,
  // Content of the editor, when using the editor as a controlled component. Incompatible with `rawContentState`.
  editorState: null,
  // Called whenever the editor state is updated. Use this to store the content of a controlled editor.
  onChange: null,
  ...
  constructor(props: Props) {
    ...
    if (editorStateProp !== null) {
      this.getEditorState = (): EditorState => {
        ...
        // $FlowFixMe If the prop is set at initialisation, assume it is going to be set later too.
        return editorState;
      };
    } else {
      this.state.editorState = conversion.createEditorState(rawContentState);
      this.getEditorState = (): EditorState => {
        const { editorState } = this.state;
        // $FlowFixMe If editorState is stored locally just above, we can assume it is accessible there.
        return editorState;
      };
    }
  }
