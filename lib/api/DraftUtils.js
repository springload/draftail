...
export default {
    ...
    /**
     * Changes depth of selected block. Only for collapsed selection.
     */
    changeBlockDepth(editorState, adjustment, maxDepth) {
        const selection = editorState.getSelection();
        const content = editorState.getCurrentContent();
    ...
    /**
     * Checks various conditions for changing depth and will accordingly
     * either call function changeBlocksDepth or just return the call.
     * https://github.com/jpuri/draftjs-utils/blob/e81c0ae19c3b0fdef7e0c1b70d924398956be126/js/list.js#L52
     */
    changeDepth(editorState, adjustment, maxDepth) {
        const selection = editorState.getSelection();
        let key;
        ...
        const withAdjustment = this.changeBlockDepth(
            editorState,
            adjustment,
            adjustedMaxDepth,
        );
    ...
    insertNewUnstyledBlock(editorState) {
        ...
        return this.removeSelectedBlocksStyle(newEditorState);
    },

    /**
     * Changes block style to unstyled for selected blocks.
    ...
    /**
     * Checks if a block is of one of the list types.
     * https://github.com/jpuri/draftjs-utils/blob/e81c0ae19c3b0fdef7e0c1b70d924398956be126/js/list.js#L13
     */
    isListBlock(block) {
        return [
            BLOCK_TYPE.UNORDERED_LIST_ITEM,
            BLOCK_TYPE.ORDERED_LIST_ITEM,
        ].includes(block.getType());
    },
    ...
    handleHardNewline(editorState) {
        ...
        if (selection.isCollapsed()) {
            ...
            if (
                !this.isListBlock(block) &&
                block.getType() !== BLOCK_TYPE.UNSTYLED &&
                block.getLength() === selection.getStartOffset()
            ) {
                return this.insertNewUnstyledBlock(editorState);
            } else if (this.isListBlock(block) && block.getLength() === 0) {
                const depth = block.getDepth();
                if (depth === 0) {
                    return this.removeSelectedBlocksStyle(editorState);
                }
                if (depth > 0) {
                    return this.changeDepth(editorState, -1, depth);
                }
            }
        }
        return undefined;
