...
export default {
    ...
    handleBeforeInputBlockType(char, block, blockTypes = []) {
        const isBlockStart = block.getLength() <= INPUT_BLOCK_MAX_LENGTH - 1;
        ...
        const newType = blockTypes.find(b => b.type === INPUT_BLOCK_MAP[mark])
    ...
    handleBeforeInputHR(char, block) {
        const isBlockStart =
            block.getLength() <= INPUT_ENTITY_MAP[HR_TYPE].length - 1;
        const mark = `${block.getText()}${char}`;

        return (
            isBlockStart &&
            block.getType() !== BLOCK_TYPE.CODE &&
            mark === INPUT_ENTITY_MAP[HR_TYPE]
        );
    },
    ...
    filterPastedContent(
        ...
        const enabledBlockTypes = blockTypes.map(block => block.type);
