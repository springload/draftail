...
import { BLOCK_TYPE, INLINE_STYLE, KEY_CODES } from '../api/constants';
...
export default {
    ...
    getKeyBindingFn(BLOCK_TYPES = [], INLINE_STYLES = []) {
        const isStyleEnabled = (arr, style) => !!arr.find(item => item.style === style);

        const getEnabledStyles = (activeStyles, allStyles) => {
            // Go through all the possible styles, and check
            return Object.keys(allStyles)
                .reduce((enabled, key) => {
                    // eslint-disable-next-line no-param-reassign
                    enabled[key] = isStyleEnabled(activeStyles, allStyles[key]);

                    return enabled;
                }, {});
        };

        const isEnabledBlock = getEnabledStyles(BLOCK_TYPES, BLOCK_TYPE);
        const isEnabledInline = getEnabledStyles(INLINE_STYLES, INLINE_STYLE);
        ...
        const keyBindingFn = (e) => {
            ...
            if (e.shiftKey || isCtrlKeyCommand(e)) {
                // Key bindings supported by Draft.js must be explicitely ignored.
                switch (e.keyCode) {
                case KEY_CODES.B:
                    return null;
                case KEY_CODES.I:
