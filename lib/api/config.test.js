...
describe('config', () => {
    ...
    describe('#getKeyBindingFn', () => {
        ...
        it('has strict keyboard shortcut matching', () => {
            expect(config.getKeyBindingFn([], [{ type: 'BOLD' }], [])({
                keyCode: KEY_CODES.B,
                metaKey: false,
                altKey: false,
                shiftKey: true,
        ...
        it('has overlapping shortcuts', () => {
            expect(config.getKeyBindingFn([{ type: 'header-five' }], [{ type: 'STRIKETHROUGH' }], [])({
                keyCode: KEY_CODES[5],
                metaKey: false,
                altKey: true,
                shiftKey: true,
        ...
        describe('blocks', () => {
            ...
            it('enables heading block key binding when required', () => {
                expect(config.getKeyBindingFn([{ type: 'header-one' }], [], [])({
                    keyCode: KEY_CODES[1],
                    metaKey: false,
                    altKey: true,
                    shiftKey: false,
        ...
        describe('entities', () => {
            ...
            it('enables link entity key binding when required', () => {
                expect(config.getKeyBindingFn([], [], [{ type: 'LINK' }])({
                    keyCode: KEY_CODES.K,
                    metaKey: false,
                    altKey: false,
                    shiftKey: false,
